...
public class BalanceMensualDS implements JRDataSource {
    ...
    public BalanceMensualDS(List<Egreso> egresos, List<IngresoOtro> ingresos) {
                
        if (!egresos.isEmpty()) {
            for (Egreso unEgreso : egresos) {
                balance.add(new Object[]{df.format(unEgreso.getFecha()), unEgreso.getUnConceptoEgreso().getNombre(), unEgreso.getObservacion(), 0, unEgreso.getMonto()});
            }
        }
        for (IngresoOtro unIngreso : ingresos) {
            balance.add(new Object[]{df.format(unIngreso.getFecha()), unIngreso.getUnConceptoIngreso().getNombre(), unIngreso.getDetalle(), unIngreso.getMonto(), 0});
        }
